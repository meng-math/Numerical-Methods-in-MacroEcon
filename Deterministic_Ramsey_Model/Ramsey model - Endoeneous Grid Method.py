...
g_old_on_k = 0.3 * f(grid_k)   # g(x) on x grid
...
grid_xp = f(grid_kp)    # grid for cash-at-hand x = f(k')
g_old_on_xp = 0.3 * grid_xp               # your original initialization
...
while err > tol:
    ...
    xs = np.empty_like(grid_xp)   # x_today
    kps = np.empty_like(grid_xp)  # k'
    ...
    g_new_on_x = np.interp( grid_x, xs, kps) # g_new(x) on x grid
    g_new_on_k = np.interp( f(grid_k), grid_x, g_new_on_x)  # g_new(k) on k grid
